...
?>	
	<title>AM - Attendance Monitor (Prototype 4)</title>
...
<body>
    <header id="cwd_header" role="banner">
        <section class="cwd_container">
            <hgroup class="grid_12" id="cwd_hgroup">
                <a href="<?php echo $base_url; ?>"><h1>Prototype 4</h1></a>
	...
	if(empty($Person_Access_Token)){
		...
		"<div class=\"notice\">" .
		"<p>Prototype 4 focuses on providing data analysis and visualisation to teaching staff for the modules they teach. Data is collected, processed
		 and analysed, providing month by month and by module analysis." .
		"</div>" . 
		"</div>";
		...
		if($Person_User_Type == "student"){
			...
			if($result['Success'] == true){ // If getNucleusData does not return an error
				...
				foreach($result2['Data']['results'] as $res){
					if((!in_array($res['cmis_course']['id'], $pDataChecker)) && (in_array($res['cmis_course']['id'], $unitArray))){
		...
		else if($Person_User_Type == "staff"){ //Staff side of the project
			echo "<div class=\"grid_12\"><h2>View attendance data by.....</h2></div>";
			if($override == true){
				$linkString = "?override=true&staff=true&dataType=";
			}
			else{
				$linkString = "?staff=true&dataType=";
			}
			echo "<a href=\"" . $base_url . $linkString . "byMonth\"><div class=\"grid_6 customLinks\"><h3>Month</h3></div></a>";
			echo "<a href=\"" . $base_url . $linkString . "byModule\"><div class=\"grid_6 last customLinks\"><h3>Module</h3></div></a>";
			...
			if($result['Success'] == true){ // If getNucleusData does not return an error
				...
				$person_units = $json['results'][0]['units']; //further array of arrays
				//$person_units = array();
				//$person_units[] = array('id' => 'CMP3022M', 'title' => 'Software Engineering');
				//$person_units[] = array('id' => 'RAN0001M', 'title' => 'Random Module One');
				asort($person_units); //sort the array
				//Get future events for both next module info and to work out attendance data later on
				$params = array(
					'access_token' => $Person_Access_Token
				...
				echo "<div class=\"grid_6 last dataBox\">";
				echo "<h3>Upcoming Events</h3><p>";
				foreach ($person_units as $unit){
					echo "<b>" . $unit['id'] . "</b> - ";
					//Check for future data in our above pData array
				...
				if($result2['Success'] == true){ // If getNucleusData does not return an error
					...
					//generate all the session and attendance data....
					//$result = generateModuleAndAttendance($mongo_db, false, $person_units, $event_results);
					...
					if($dataType == "bymodule"){
						//Module analysis
						overallAttendanceByModule($mongo_db, $moduleArray);
					}
					else{
						//Month by month analysis
						$dateDetails = getAcademicDates();
						$month_dates = $dateDetails['month_dates'];
						overallAttendanceByMonth($mongo_db, $moduleArray, $month_dates);
					}		
				}
				else{		
					echo "<h2>" . $result2['Error_Type'] . "</h2>";
			...
			echo "<div class=\"notice\"><h1>Eh?</h1>You don't appear to be a student or member of staff at the University of Lincoln.</div>";
		}
	} ?>
    </section>
