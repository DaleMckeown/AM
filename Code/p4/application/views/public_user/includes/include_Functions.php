...
/* Lecturer Functions ********************************************/
/*****************************************************************/
function overallAttendanceByMonth($mongo_db, $moduleArray, $monthArray){
	/****** Start Week by Week Analysis ******/
	$collection = $mongo_db->am_user_events; // Change to MongoDB relevant collection
	$current_time = time();
	echo "<h2>Month by Month Attendance</h2>";
	...
	foreach($monthArray as $month){
		...
		if($current_time >= $month['Month_Start']){
			...
			foreach($moduleArray as $module){
				//***** Pull data from database, increment counters, output result. *****/
				$cursor = $collection->find(array('Module_ID' => $module['Module_ID'], 'Start_Time' => array('$gte' => $month['Month_Start'], '$lte' => $month['Month_End'])));
				$cursorArray = iterator_to_array($cursor, false); // 'use_keys' must be false.						
				if(!empty($cursorArray)){
					foreach($cursorArray as $lecture){
						$sessionArray[] = $lecture;
			...
			//output each session and increment counters
			echo "<table><tr>" .
			"<th>Module</th><th>Time</th><th>Attended</th><th>%</th>" . 
			"</tr>";
			
			foreach ($sessionArray as $session){
				$sessionPeopleCount = 0;
				$sessionPeopleAttendedCount = 0;
				$sessionPeopleAbsentCount = 0;
				foreach($session['Attended'] as $attendance){ //each students attendance
					$sessionPeopleCount++;
					if($attendance == 1){
						$sessionPeopleAttendedCount++;
					}
					else{
						$sessionPeopleAbsentCount++;
					}
				}
				$sessionAttendancePercentage = round(($sessionPeopleAttendedCount / $sessionPeopleCount) * 100, 1);
				echo "<tr>" .
				"<td>" . $session['Module_ID'] ."</td>" .
				"<td>" . customDateShort($session['Start_Time'], $session['End_Time']) . "</td>" . 
				"<td>" . $sessionPeopleAttendedCount . "/" . $sessionPeopleCount . "</td>" . 
				"<td>" . $sessionAttendancePercentage . "%</td>" .
				"</tr>";
				//increment counters
				$monthLectureCount++;
				$monthLecturePeopleCount += $sessionPeopleCount;
				$monthAttendedCount += $sessionPeopleAttendedCount;
				$monthAbsentCount += $sessionPeopleAbsentCount;
			}
			echo "</table>";
			echo "<p>" . $monthLectureCount . " sessions, " . $monthLecturePeopleCount . " session slots<br><font class=\"Green\">" . $monthAttendedCount . " session slots attended</font><br><font class=\"Red\">" . $monthAbsentCount . " session slots absent</font></p>";
			...
			//add counters
			$overallLectureCount += $monthLectureCount;
			$overallLecturePeopleCount += $monthLecturePeopleCount;
			$overallAttendedCount += $monthAttendedCount;
			$overallAbsentCount += $monthAbsentCount;
			
			//Calculate percentage
			if($monthLectureCount > 0){
				$attendancePercentage = ($monthAttendedCount / $monthLecturePeopleCount) * 100;
				$absentPercentage = 100 - $attendancePercentage;
			}
			//final graph data array
			$monthDataArray[] = array(
	...
	echo "<div class=\"grid_12  dataBox\">";
	echo "<h2>Overall Data</h2>";
	$attendancePercentage = round(($overallAttendedCount / $overallLecturePeopleCount) * 100, 2);
	echo "<p>" . $overallLectureCount . " sessions, " . $overallLecturePeopleCount . " session slots<br><font class=\"Green\">" . $overallAttendedCount . " session slots attended</font><br><font class=\"Red\">" . $overallAbsentCount . " session slots absent</font></p>";
...
function overallAttendanceByModule($mongo_db, $moduleArray){
	$collection = $mongo_db->am_user_events; // Change to MongoDB relevant collection
	$current_time = time();
	echo "<h2>Attendance by Module</h2>";
	//Variables to control overall figures
	...
	foreach($moduleArray as $module){
		...
		//***** Pull data from database, increment counters, output result. *****/
		$cursor = $collection->find(array('Module_ID' => $module['Module_ID']));
		$cursorArray = iterator_to_array($cursor, false); // 'use_keys' must be false.						
		$sessionArray = array();
		if(!empty($cursorArray)){
			foreach($cursorArray as $lecture){
			...
			echo "<h4>" . $lastMonth . "</h4>";
			echo "<table><tr>" .
			"<th>Time</th><th>Attended</th><th>%</th>" . 
			"</tr>";
			$count = 1; //1 because of the above div creation
			foreach ($sessionArray as $session){
				if($lastMonth != date("F", $session['Start_Time'])){
				...
				$sessionPeopleAttendedCount = 0;
				$sessionPeopleAbsentCount = 0;
				foreach($session['Attended'] as $attendance){ //each session
					$sessionPeopleCount++;
					if($attendance == 1){
						$sessionPeopleAttendedCount++;
					}
				...
				//increment counters
				$moduleLectureCount++;
				$moduleLecturePeopleCount += $sessionPeopleCount;
				$moduleAttendedCount += $sessionPeopleAttendedCount;
				$moduleAbsentCount += $sessionPeopleAbsentCount;
			...
			echo "<div class=\"overallStatContainer\">" .
			"<h4>Module Analysis</h4>";
			echo "<p>" . $moduleLectureCount . " sessions, " . $moduleLecturePeopleCount . " session slots<br><font class=\"Green\">" . $moduleAttendedCount . " session slots attended</font><br><font class=\"Red\">" . $moduleAbsentCount . " session slots absent</font></p>";
			...
			//add counters
			$overallLectureCount += $moduleLectureCount;
			$overallLecturePeopleCount += $moduleLecturePeopleCount;
			$overallAttendedCount += $moduleAttendedCount;
			$overallAbsentCount += $moduleAbsentCount;
			
			//Calculate percentage
			$attendancePercentage = ($moduleAttendedCount / $moduleLecturePeopleCount) * 100;
			$absentPercentage = 100 - $attendancePercentage;
